...
for(const asset of ['style.css','editorial.css','app.js','editorial.js'])assetHash.update(await fs.readFile(path.join(root,'src/assets',asset)));
...
await write('index.html',shell({title:'Agentic Engineering — The Field Guide',description:'Twelve deep dives into reliable AI systems, from experiments and agents to inference, ontologies, and cost per accepted outcome.',body:home,className:'home'}));
...
const bookBody=`<header class="chapter-header"><div class="eyebrow">FIELD GUIDE / READING EDITION</div><h1>Agentic Engineering</h1><p class="chapter-subtitle">All 12 chapters in one place. Use your browser’s print command to save a PDF.</p><p><a href="${base}/downloads/agentic-engineering.md" download>Download Markdown ↓</a></p></header><article class="prose book-prose">${render(combined)}</article>`;
...
await write('sitemap.xml',`<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">${[`${base}/`,...chapters.map(c=>c.href),`${base}/progress/`,`${base}/capstone/`,`${base}/glossary/`,`${base}/sources/`].map(u=>`<url><loc>${origin}${u}</loc></url>`).join('')}</urlset>`);
